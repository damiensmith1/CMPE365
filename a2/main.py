...
# returns the number of adjacent triangles that a triangle has
# used to find starting point, best next triangle and to see if there are available moves left
def availableTris(triangle):
    num = 0
    for i in triangle.adjTris:
        if i.nextTri is None and i.prevTri is None:
            num += 1
    return num
...
def buildTristrips(triangles):
    # because buildTristrips is recursive, this is needed to keep track of the number of strips
    global count
    count += 1 

    trianglesCopy = triangles.copy()

    # used to check if the value was changed - if unchanged then it means 
    # that there were no more available triangles to choose from
    curr = "no legal moves" 
    smallest = 4 # set the maximum number of adjacent triangles that the triangle can have + 1
    # find starting point by looping through the list, checking if they are already in a strip, and 
    # looking at their adjacent triangles to find which is has the smallest number of adjacent triangles
    for i in trianglesCopy: 
        if i.nextTri is None and i.prevTri is None:
            for j in i.adjTris:
                if availableTris(j) < smallest: # if the number of adjacent triangles is smaller than the previous smallest
                    if j.nextTri is None and j.prevTri is None:
                        smallest = availableTris(j) # set a new smallest tri
                        curr = j # the "current best starting triangle" is updated
    trianglesCopy.remove(i)
    # if curr is zero that means that it the for loop wasnt able to find a triangle and 
    # there are no longer any available triangles in the list
    if curr == "no legal moves": 
        return 0
    
    curr.highlight1 = True
    
    # builds the strip until no available moves are left
    while 1:
        # if the current triangle has no more available adjacent triangles to point to -> break
        if availableTris(curr) == 0:
            break
        # same logic as finding the best starting point - from current triangle, loop 
        # through the adjacent triangles to find out which has the smallest number of adjacent triangles
        best = 4
        for k in curr.adjTris:
            if availableTris(k) < best:
                if k.nextTri is None and k.prevTri is None:
                    best = availableTris(k)
                    bestTri = k # set the best triangle option
                    
        # set pointers to build out the strip
        curr.nextTri = bestTri
        curr = curr.nextTri 
        curr.prevTri = curr
    
    print('Generated %d tristrips' % count)
    # recurse to find a new starting point to build a new strip
    buildTristrips(trianglesCopy)
