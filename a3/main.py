...
def buildTriangles( slice0, slice1 ):
    ...
    #
    # This can be done with "brute force" if you wish. - O(n^2)
    #
    # [1 mark] 

    # [YOUR CODE HERE]

    # finding the smallest distance between verticles in the slices using brute force method
    # set a starting "smallest" from the first 2 points
    smallest = length(subtract(slice0.verts[0].coords, slice1.verts[0].coords))
    # initalize indexes to set them later
    indexS0 = 0
    indexS1 = 0

    # finding the smallest distance vertexes brute force O(n^2) - returns the index of the shortest distance vertexes
    for i in range(len(slice0.verts)):
        for j in range(len(slice1.verts)):
            # get the distance between the points
            current = length(subtract(slice0.verts[i].coords, slice1.verts[j].coords))
            # if the current distance is shorter than the shortest one then set it as the smallest
            if current < smallest:
                smallest = current
                indexS0 = i
                indexS1 = j
    ...
    # store the the closest vertex in each slice and everything after it then add everything before it (index + 1)
    # including the closest so that it ends on the same edge it starts
    cycle0 = slice0.verts[indexS0:] + slice0.verts[:indexS0 + 1]
    cycle1 = slice1.verts[indexS1:] + slice1.verts[:indexS1 + 1]
    ...
    # create 2d array of 'None' elements with size of the slice + 1 to account for the repeating starting vertex
    minArea = [[None for unused in range(len(slice0.verts) + 1)] for unused2 in range(len(slice1.verts) + 1)]
    minDir = [[None for unused in range(len(slice0.verts) + 1)] for unused2 in range(len(slice1.verts) + 1)]
    ...
    # [YOUR CODE HERE]
    # going left on the table
    for col in range(len(cycle0) - 1 ):
        # compute the area of the triangle to add to the minArea and set the direction
        area = triangleArea(cycle0[col].coords, cycle0[col + 1].coords, cycle1[0].coords)
        minDir[0][col + 1] = Dir.PREV_COL
        minArea[0][col + 1] =  area + minArea[0][col]
    ...
    # [YOUR CODE HERE]
    # going up 
    for row in range(len(cycle1) - 1 ):
        # compute the area of the triangle to add to the minArea and set te direction
        area = triangleArea(cycle1[row].coords, cycle1[row + 1].coords, cycle0[0].coords)
        minDir[row + 1][0] = Dir.PREV_ROW
        minArea[row + 1][0] = area + minArea[row][0]
    ...
    # [YOUR CODE HERE]
    # loop through the rows and cols and get the area of both triangle options - then compare them and set the minumum area accordingly
    for row in range(len(cycle1) - 1):
        for col in range(len(cycle0) - 1):
            # compute the area of the triangles from above and to the left on the table
            left = triangleArea(cycle0[col].coords, cycle0[col + 1].coords, cycle1[row + 1].coords) + minArea[row + 1][col]
            above = triangleArea(cycle1[row].coords, cycle1[row + 1].coords, cycle0[col + 1].coords) + minArea[row][col + 1]
            # check which triangle (from left or above it) has greater area and set the min area accordingly with the direction
            if left < above:
                minDir[row + 1][col + 1] = Dir.PREV_COL
                minArea[row + 1][col + 1] = left
            else:
                minDir[row + 1][col + 1] = Dir.PREV_ROW
                minArea[row + 1][col + 1] = above
    ...
    # loop "backwards" starting in the bottom right

    row = len(minArea) - 1
    col = len(minArea[0]) - 1

    # loop from maximum until either the row or col are 0
    while row != 0 or col != 0:
        # depending on the direction (horizontal or vertical) calcualted from the previous
        # if it was from row, then add the triangle and move up
        # if it was col, then add the triangle and move to the left
        if minDir[row][col] == Dir.PREV_COL:
            # create the triangle and append it to the array
            triangles.append(Triangle([cycle0[col], cycle0[col-1], cycle1[row]]))
            col -= 1
        elif minDir[row][col] == Dir.PREV_ROW:
            # create the triangle and append it to the array
            triangles.append(Triangle([cycle1[row], cycle1[row-1], cycle0[col]]))
            row -= 1
